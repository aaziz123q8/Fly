...
{
    private const KEYS = ['site_email', 'site_phone', 'site_whatsapp', 'site_logo'];

    private static function readAll(): array
    {
        $out = ['site_email' => '', 'site_phone' => '', 'site_whatsapp' => '', 'site_logo' => ''];
        try {
    ...
    public function update(Request $request): void
    {
        ...
        foreach (self::KEYS as $k) {
            $v = $request->input($k);
            if ($v === null) continue;
            $ins->execute([$k, trim((string) $v), $labels[$k] ?? $k]);
        }
